...
Add dice loss in this file by Xing @ 12sigma
...
def dice_loss(prediction, target):
    ...
    # target = (target > 10).float()

    i_flat = prediction.view(-1)

    t_flat = target.view(-1)
    # print('sum value:',i_flat.sum(),t_flat.sum(),max(i_flat))
...
def dice_loss_focal(prediction, target):
    smooth = 1.0
    weights = target.view(-1)
    i_flat = prediction.view(-1)
    t_flat = (target > 0).view(-1).float()
    intersection = (i_flat * t_flat * weights).sum()
    union = (i_flat + t_flat) * weights
    return 1 - ((2. * intersection + smooth) / (union.sum() + smooth))
...
def Jaccard(prediction, target):
    ...
    return (intersection + smooth) / (i_flat.sum() + t_flat.sum() -intersection + smooth)
